...
public abstract class TowerScript : MonoBehaviour, IRuntimeMovableBehaviourScript
{
    ...
    /// <summary>
    /// Update is called by each tower on every frame update, to update their current target(s) and display the range visual if it is toggled on.
    /// </summary>
    public void Update()
    {
        UpdateTargets();
    ...
    public void EnterBuddyMode(){
        fireInterval = baseFireInterval * 0.5f;
